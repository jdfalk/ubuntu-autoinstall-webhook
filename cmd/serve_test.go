...
// withMockInitDB temporarily replaces the exported InitDBFunc with a mock function.
func withMockInitDB(mock func() error, testFunc func()) {
	originalInitDBFunc := InitDBFunc
	InitDBFunc = mock
	defer func() { InitDBFunc = originalInitDBFunc }()
	testFunc()
}

func TestServeCommand(t *testing.T) {
	// Use the shared testutils.MockDBInit to simulate successful database initialization.
	withMockInitDB(testutils.MockDBInit(), func() {
		withMockStartServer(mockStartServer, func() {
			...
			t.Run("Serve command execution", func(t *testing.T) {
				...
				// Check that the expected log output is present.
